<?php
    //THE VERIFICATION CODE STARTS AT LINE 44
    session_start();
include ('../includes/db-conn.php');

    if (isset($_SESSION['ve-status'])){
...
<?php
    if (isset($_POST['submit'])){
        ...
        $expected = $_SESSION['expected_code'];
        $ver = "yes";
        $sess_email = $_SESSION['user_email'];
        ...
        if ($code != $expected) {
            ...
            unset($_SESSION['ve-status']); //unset the verify status for security reasons
            //update
                $dropp = mysqli_query($conn, "UPDATE users
                SET verified='$ver' WHERE email= '$sess_email' "
                );
            
            //fetch details from db
                $email_check = mysqli_query($conn, "SELECT * FROM users WHERE email = '$sess_email' ");
                //This line fetches the result from mysql as an array
                $email_check_array = mysqli_fetch_array($email_check);

            //set sign in sessions here
            ...
            header("Location: ../user-dashboard");
                        ...
                        $mailHeaders = "Kindly enter the code below in Note9 Verification Page."  .
                        "\r\n OTP: ". $random_number ;

                        if(mail($toEmail, $uname, $mailHeaders)) {
                            $message = "Done";   //Useless piece of code
                            $_SESSION['expected_code'] = $random_number;  //Updates the expected code session
...
<input type="text" class="form-control" name="otp_entered" placeholder="Enter 5 digit Code" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" minlength="5" maxlength="5">
